...
# Draw plot
plt.figure(dpi= 80)
plt.hlines(y=df['color'], xmin=0, xmax=df['s_price'], color=df['colors'], alpha=0.4, linewidth=5)

# Decorations
plt.gca().set(ylabel='$Color$', xlabel='$Price$')
plt.yticks(df['color'], fontsize=12)
plt.title('Diverging Bars of diamonds\'s price', fontdict={'size':20})
plt.grid(linestyle='--', alpha=0.5)
plt.show()

plt.savefig('Diverging_Bar_mpl.png')